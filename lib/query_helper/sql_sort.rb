...
class QueryHelper
  class SqlSort
    ...
    def attributes_sql_expression(sort_attribute)
      ...
      puts @column_sort_order
      sql_strings << parse_custom_sort_string if @column_sort_order.present?
      sql_strings
    end

    # This method is used for sorting enum based column
    def parse_custom_sort_string
      return '' unless @column_sort_order.present? || @column_sort_order[:sort_values].present?
